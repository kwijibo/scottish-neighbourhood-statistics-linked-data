...
$LaGraph->add_turtle(file_get_contents('output-data/local-authorities.ttl'));
  ...
  foreach($LaGraph->get_subject_property_values($s, OWL_SAMEAS) as $o)
  {
    ...
    foreach($sameAsIndex as $s_s => $s_ps){
      if(isset($s_ps[OWL_SAMEAS])){
        if(strpos($s_s, 'http://data.')===0) $LaGraph->add_resource_triple($s, OWL_SAMEAS, $s_s);          
        foreach($s_ps[OWL_SAMEAS] as $s_o){
        if(strpos($s_o['value'], 'http://data.')===0) $LaGraph->add_resource_triple($s, OWL_SAMEAS, $s_o['value']);          
        }
      }
    }
  }
...
//file_put_contents('output-data/local-authorities-with-os-sameAs.ttl', $LaGraph->to_turtle());
//file_put_contents('/output-data/local-authorities-with-os-sameAs.nt', $LaGraph->to_ntriples());

echo $LaGraph->to_ntriples();
?>
